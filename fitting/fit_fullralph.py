...
def cat(iCat,iHP,iHF,iBin,iBase,iPt,iFunc):
    ...
    lQCD  = histFunc([iHP[len(iHP)-2],iHF[len(iHF)-2]],iBase,"qcd",iBin)
...
def baseVars(iPtBins):
    ...
    lVars=[lMSD,lEff,lEffQCD,lDM,lShift,lPt,lRho]
...
def makeRalph(iHs,iBase,iPt,iCat):
    ...
    for i0 in range(1,iHs[0].GetNbinsX()+1):
        ...
        lPass = command(iBase[0].getVal(),iBase[6].getVal(),lUnity,lVars,lNR,lNP,lNRP)
        pSum = 0
...
def makeFullRalph(iHP,iHF,iBin="cat"):
    ...
    for pt in range(1,lPtBins+1):
        ...
        lParHists = makeRalph([lFCat[0],lFCat[1],lFCat[2]],lBase,pPt,iBin+str(pt))
...
def load(iFileName,iHP,iHF,i2D=True,iPseudo=False):
    end=""
    if i2D:
        end="_2D"
