...
def main():
	...
	txtc = ROOT.TLatex(0.68,0.92,"2.7 fb^{-1} (13 TeV)");
	...
	leg2 = ROOT.TLegend(0.48,0.20,0.73,0.40);
	leg2.SetFillStyle(0);
	...
	leg2b = ROOT.TLegend(0.13,0.68,0.62,0.81);
	...
	hrl2 = can_gB.DrawFrame(75,0.05,800,1);
...
def makeAGraph(listx,listy,linecolor = 1, linestyle = 1):
	...
	for i in range(len(listx)):
		a_m.append(listx[i]);
		a_g.append(listy[i]/6.);  # need factor for conversion to g_q
...
def makeAFillGraph(listx,listy1,listy2,linecolor = 1, fillcolor = 0, fillstyle = 0):
	...
	for i in range(len(listx)):
		a_m.append(listx[i]);
		a_g.append(listy1[i]/6.); # need factor for conversion to g_q
	
	for i in range(len(listx)-1,-1,-1):
		a_m.append(listx[i]);
		a_g.append(listy2[i]/6.); # need factor for conversion to g_q
